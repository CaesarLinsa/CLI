...
class shellmain(object):
    ...
    def __init__(self, module, prog, epilog=None):
        self.module = module
        self.prog = prog
        self.epilog = epilog
        super(shellmain).__init__()

    def get_base_parse(self):
        parser = argparse.ArgumentParser(
            prog=self.prog,
            description='',
            epilog=self.epilog if self.epilog else
            "see %s for help" % self.prog,
            add_help=False
        )
    ...
    def get_subcommand_parser(self):
        ...
        sub_modules = self.import_modules(self.module)
...
def main(module,
    ...
    if argv is None:
        argv = sys.argv[1:]
    shellmain(module, prog, epilog).main(argv)
